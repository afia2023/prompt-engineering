# Re-create the grouped LLM family trend chart from Afia's table (2020–2025), then show + save it.
import matplotlib.pyplot as plt
from collections import OrderedDict
import numpy as np
from PIL import Image

# 1) Year per reference
year_by_ref = {
...
# 2) LLMs per paper (from your table)
llms_by_ref = {
...
# --- replace your plotting loop with this ---
# --- replace your plotting loop with this ---
plot_order = [f for f in [
    "GPT family","Llama family","CodeT5 family","DeepSeek family",
    "StarCoder / StarChat","PLBART","InCoder","CodeBERT","ERNIE-Code",
    "Gemma family","Codestral","GPT-Neo"
] if f in fam_counts]

# small per-family x-offsets so markers don’t sit on top of each other
offsets = np.linspace(-0.15, 0.15, len(plot_order))

for i, fam in enumerate(plot_order):
    y = np.array(fam_counts[fam], dtype=float)
    # jitter the x slightly and use bigger, outlined markers
    plt.plot(
        x + offsets[i], y,
        marker='o', markersize=7, linewidth=1.6, label=fam,
        markeredgecolor='white', markeredgewidth=1.25, zorder=3
    )

# === Put this BELOW the plotting loop ===
plt.ylim(-0.2, max(max(v) for v in fam_counts.values()) + 1)

plt.axvline(2022.9, color='gray', linestyle='--', linewidth=1)
plt.text(2023.02, max(max(v) for v in fam_counts.values())*0.85, "ChatGPT released\nNov 2022",
...
# axis labels remain
plt.xlabel("Year"); plt.ylabel("Mentions in studies (per family)")
